...
class ListPage : BaseContentPage<ListViewModel>
{
	...
	async void HandleSelectionChanged(object? sender, SelectionChangedEventArgs e)
	{
		...
		if (e.CurrentSelection.FirstOrDefault() is LibraryModel libraryModel)
		{
			...
			var detailsPage = (DetailsPage)Navigation.NavigationStack.Single(x => x is DetailsPage);
			detailsPage.DetailsPageDisappearing += HandleDetailsPageDisappearing;
		}

		collectionView.SelectedItem = null;
	}

	async void HandleDetailsPageDisappearing(object? sender, EventArgs e)
	{
		await Toast.Make("Details Page Disappeared").Show();
	}
