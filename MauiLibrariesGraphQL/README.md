...
```bash
MauiLibrariesGraphQL$ sam build
```
...
```bash
MauiLibrariesGraphQL$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
MauiLibrariesGraphQL$ sam local start-api
MauiLibrariesGraphQL$ curl http://localhost:3000/
```
...
Resources:
  ...
  NetCodeWebAPIServerless:
    ...
    Properties:
      CodeUri: ./MauiLibrariesGraphQL/
      Handler: MauiLibrariesGraphQL::MauiLibrariesGraphQL.Function::FunctionHandler
      Runtime: dotnet6
      MemorySize: 1042
      DeadLetterQueue:
        Type: SQS
...
```bash
MauiLibrariesGraphQL$ sam build
MauiLibrariesGraphQL$ sam deploy
```
...
```bash
MauiLibrariesGraphQL$ sam logs -n NetCodeWebAPIServerless --stack-name MauiLibrariesGraphQL --tail
```
...
```bash
MauiLibrariesGraphQL$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
...
```bash
sam delete --stack-name MauiLibrariesGraphQL
```
